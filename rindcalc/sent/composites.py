...
def bathymetric(sent_dir, out_composite):
    ...
    red_band = norm(red.GetRasterBand(1).ReadAsArray().astype(np.uint16),
                    255, 0)
    green = gdal.Open(bands[2])
    green_band = norm(green.GetRasterBand(1).ReadAsArray().astype(np.uint16),
                      255, 0)
    coastal = gdal.Open(bands[0])
    coastal_band = norm(coastal.GetRasterBand(1).ReadAsArray().astype(np.uint16)
                        , 255, 0)
    snap = gdal.Open(bands[3])
